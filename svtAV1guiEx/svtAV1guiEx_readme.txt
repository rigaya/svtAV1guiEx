...
【どうでもいいメモ】
2026.09.02 (2.15)
- 実行ファイルのバージョンチェックを更新し、古いバージョンを選択してしまわないように。
- 同梱の実行ファイルを更新。(4.2.0-107_x64)

2026.08.25 (2.14)
- システム全体のハンドル数が非常に多い環境でのエラー回避。
- 同梱の実行ファイルを更新。(4.2.0-84_x64)

2026.08.14 (2.13)
- VBR指定が2.11から正常に動作しなかった問題を修正。
- 同梱の実行ファイルを更新。(4.2.0-76_x64)
...
2025.12.05 (2.07)
- 2passエンコード時の中間ファイル削除の修正。
- 同梱の実行ファイルを更新。(3.1.0-198_x64)

2025.11.04 (2.06)
- --ac-biasの設定欄を追加。
- 同梱の実行ファイルを更新。(3.1.0-185_x64)

2025.10.16 (2.05)
- 音声の32bit float出力で処理がフリーズする可能性があったのを修正。
- svt-av1のcrfの最大値拡大に対応。
- clang版svt-av1でpreset=2以下の10bitエンコード時に強制終了する問題を回避。
  clangでのビルド時にstackサイズを32MBに強制的に拡大して、スタックオーバーフローを回避。
- 同梱の実行ファイルを更新。(3.1.0-157_x64)
