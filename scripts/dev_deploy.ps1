...
<#
.SYNOPSIS
  svtAV1guiEx をビルドし、AviUtl2 Plugin へ配置して起動する。
...
$Sln = Join-Path $RepoRoot "svtAV1guiEx.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\x64\svtAV1guiEx.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\Win32\svtAV1guiEx.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "svtAV1guiEx.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
...
foreach ($name in @("svtAV1guiEx.ini", "svtAV1guiEx.en.ini", "svtAV1guiEx.zh.ini")) {
