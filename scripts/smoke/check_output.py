#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""svtAV1guiEx 終了バッチ用: 出力ファイル / ログ / aup2 設定のスモークチェック。"""
...
def extract_log_svtav1_options(log_text: str) -> str | None:
    lines = log_text.splitlines()
    for idx, line in enumerate(lines):
        if "svt-av1 options" in line:
            buf: list[str] = []
            for fol in lines[idx + 1 :]:
                ...
                if not s:
                    ...
                    continue
                # 次セクションっぽい行で打ち切り
                if re.match(r"^(AAC|raw |svt-av1|L-SMASH|CPU|encoded |converting |Aviutl)", s):
                    break
                if s.startswith("---"):
                    break
                buf.append(s.strip())
...
def compare_cli(expected: str, actual: str) -> list[str]:
    ...
    if not expected:
        return problems
    if not actual:
        problems.append("ログから svt-av1 options を抽出できませんでした")
        return problems
...
def main() -> int:
    ap = argparse.ArgumentParser(description="svtAV1guiEx smoke check (after-bat)")
    ...
    name_keys = cfg.get("plugin_name_contains") or ["svtAV1guiEx"]
    ...
    pprint("=== svtAV1guiEx smoke check ===")
    ...
    if not logpath.is_file():
        ...
        log_cli = extract_log_svtav1_options(log_text)
        if log_cli:
            ok(f"ログ CLI 抽出: {log_cli[:120]}{'...' if len(log_cli) > 120 else ''}")
        else:
            ng("ログから svt-av1 options を抽出できませんでした")
        ...
        if aup2 and aup2.is_file():
            try:
                ...
                if conf is None:
                    ng(f"aup2 から svtAV1guiEx 設定を見つけられません: {aup2}")
                else:
